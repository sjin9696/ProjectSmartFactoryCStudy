//로또 팀 프로젝트
//팀장 : 1. 서주형
//팀원 : 2. 이은수, 3. 강춘만, 4. 이승원, 5. 송진영
//목적 로또 프로그램 필요한 함수만 다시 정리 하기.
...
#define FIRST_th_PER 1566 
#define SECOND_th_PER 2826
#define THIRD_th_PER 1128
#define SHUTDOWN_TIME_SEC 3 
#define LOTTO_BOX_SIZE 10000

#define LOTTO_ROUND 920
#define LOTTO_SIX_NUMBER 6
#define LOTTO_1to45_LENTH 45

#define LOTTO_MACHIN_1_3TH 1,920 //1세대 261 
#define LOTTO_MACHIN_1TH 1,261 //1세대 261 
#define LOTTO_MACHIN_2TH 262,732 //2세대 471 
#define LOTTO_MACHIN_3TH 733,920 //3세대 188 


typedef struct T_LottoWinningNumber {
	int Round;
	int SixNumber[LOTTO_SIX_NUMBER];
}T_LottoWinningNumber;
...
//1.변수 선언
//함수 프로토타입 선언
void VIEW_Program_Title();
void VIEW_Menu();
void VIEW_Back_Menu();
void VIEW_Ticket_Buy();

void Menu_SELECT(int m_cost);
void Lotto_ticket_buy(int cost);
void number_compare1();
void number_compare2();
void IS_Print_Nth_Lotto(int m_first, int m_last);
void IS_Swap(int m_a, int m_b);
void IS_Select_Sorting_Ascending(int m_Array[]);
void IS_PRINT_Array(int m_Array[], int size);
void Get_Numbers_Do_Not_Overlap(int member_Array[], int m_range);
void getDuplicateLottoNumber(int m_lottoArray[], int m_range);
void SET_Lotto45_Distribution_Lookup(int m_start_small, int m_end_big);
void WORKING(int m_start_small, int m_end_big);
T_LottoWinningNumber GETLottoWinningNumberLookup(int lotto_round);
T_Box GETLotto45DistributionLookup(int lotto_round);
T_Box SETLotto45DistributionLookup_Ball(int ball_num, int count);

void VIEW_Program_Title()
{
	system("mode con:cols=76");
	system("title 이상한 로또 프로그램");
	system("cls");
	printf(RED"■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■\n");
	printf("■□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□■\n");
	printf("■□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□■\n");
	printf("■□□□□□□□□"RESET); printf(BLUE"■"RESET); printf(RED"□□□□□□□□□"RESET); printf(BLUE"■"RESET);
	printf(RED"□□□"RESET); printf(BLUE"■"RESET); printf(RED"□□□□□□□□□□□□□■\n");
	printf("■□□□□□□□□"RESET); printf(BLUE"■"RESET); printf(RED"□□□□□□□□"RESET); printf(BLUE"■■■"RESET);
	printf(RED"□"RESET); printf(BLUE"■■■"RESET); printf(RED"□□□□□□□□□□□□■\n");
	printf("■□□□□□□□□"RESET); printf(BLUE"■"RESET); printf(RED"□□□□□"RESET); printf(CYAN"■■"RESET);
	printf(RED"□□"RESET); printf(BLUE"■"RESET); printf(RED"□□□"RESET); printf(BLUE"■"RESET); printf(RED"□□"RESET);
	printf(GREEN"■■"RESET); printf(RED"□□□□□□□□□■\n");
	printf("■□□□□□□□□"RESET); printf(BLUE"■"RESET); printf(RED"□□□□"RESET); printf(CYAN"■"RESET); printf(RED"□□"RESET);
	printf(CYAN"■"RESET); printf(RED"□"RESET); printf(BLUE"■"RESET); printf(RED"□□□"RESET); printf(BLUE"■"RESET);
	printf(RED"□"RESET); printf(GREEN"■"RESET); printf(RED"□□"RESET); printf(GREEN"■"RESET); printf(RED"□□□□□□□□■\n");
	printf("■□□□□□□□□"RESET); printf(BLUE"■■■■■"RESET); printf(RED"□"RESET); printf(CYAN"■■"RESET); printf(RED"□□"RESET);
	printf(BLUE"■■"RESET); printf(RED"□□"RESET); printf(BLUE"■■"RESET); printf(RED"□"RESET); printf(GREEN"■■"RESET);
	printf(RED"□□□□□□□□□■\n");
	printf("■□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□■\n");
	printf("■□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□□■\n");
	printf("■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■■\n");
	printf(RESET); printf(GREEN); // 글자색 변경
	putchar('\n');
}
void VIEW_Menu() //이거는 나중에 VIEW_MAIN으로 갈 방안 찾기.
{
	int menuSelect = 0;
	VIEW_Program_Title();
	printf(CYAN"\n"); // 글자색 변경
	printf("\t[1]\t─────── 1세대부터 3세대까지 확률 보기\t──────\n\n");
	printf("\t[2]\t─────── 1세대랑 3세대 비교\t\t──────\n\n");
	printf("\t[3]\t─────── 2세대랑 3세대 비교\t\t──────\n\n");
	printf("\t[4]\t─────── 로또 티켓 구매하기\t\t──────\n\n");
	//printf("5 ------ - 할이리 리스트\n\n");
	printf("\n");
	printf("\t선택하실 메뉴를 골라주세요 : ");
	scanf_s(" %d", &menuSelect);
	Menu_SELECT(menuSelect);
}
void VIEW_Program_Shutdown()
{
	printf("\n\n종료합니다.\n");
	getchar();
	exit(1); //stdlib.h 프로그램 정상 종료 함수 호출
	return;
}
void VIEW_Back_Menu() {
	int key = 0;
	...
	case 'X':
	case 'x':
		VIEW_Program_Shutdown();
		break;
	default:
		printf("\t잘못된 입력입니다. 메뉴에서 선택 하세요.\n");
		VIEW_Back_Menu();
	}
}
void VIEW_Ticket_Buy() {
	int cost = 0;
	printf("\n");
	printf("\t1장의 금액은 1000원입니다. \n\n");
	printf("\t투입 금액 입력 : ");
	scanf_s("%d", &cost);
	Lotto_ticket_buy(cost);
}
void Lotto_ticket_buy(int cost) {
	int max_ticket = 0;
	int buy_ticket = 0;
	int lottoArray[LOTTO_SIX_NUMBER] = { 0 }; //로또 배열 선언
	if (cost < 1000) {
		printf("\n\t금액이 부족합니다. \n\n");
		VIEW_Ticket_Buy();
	}
	else {
		max_ticket = cost / 1000;
		printf("\n\t최대 장수는 %d장 입니다. \n\n", max_ticket);
		printf("\t구매 장수 입력 : ");
		scanf_s("%d", &buy_ticket);
		if (buy_ticket > max_ticket) {
			printf("\n\t금액이 부족합니다. \n\n");
			VIEW_Ticket_Buy();
		}
		else if (max_ticket >= buy_ticket) {
			for (int i = 0; i < buy_ticket; i++) {
				printf("\n\t[%d] 로또 번호 출력 ─── ★ ", i + 1);
				WORKING(LOTTO_MACHIN_3TH);
				printf(" ★\n");
			}
			if (cost - (buy_ticket * 1000) == 0) {
			}
			else if (cost - (buy_ticket * 1000) != 0) {
				printf("\n\t남은 잔금 %d원을 반환합니다. \n", cost - (buy_ticket * 1000));
			}
		}
		printf("\n\n\t희망로또번호 추출 완료 ! \n");
		printf("\n");
...
void IS_PRINT_NEWLINE(int n_repeat, int times) {
	if (n_repeat % times == times - 1)
	{
		printf("\n");
	}
}
...
void IS_PRINT_Array(int m_Array[], int size) {
	//printf("size : %d\n", size);
	//[11], [17], [21], [26], [36], [45]
	/*printf("\n\t[2] 로또 번호 출력 ─── ★ [ 4], [ 8], [ 9], [16], [17], [19] ★\n");
	printf("\n\t[3] 로또 번호 출력 ─── ★ [ 5], [21], [27], [34], [37], [40] ★\n");
	printf("\n\t[4] 로또 번호 출력 ─── ★ [ 7], [ 9], [12], [14], [23], [28] ★\n");
	printf("\n\t[5] 로또 번호 출력 ─── ★ [ 3], [12], [14], [23], [28], [41] ★\n");*/
...
void IS_Print_Nth_Lotto(int m_start_small, int m_end_big)
{
	//3가지 기능 합쳐져 있어서 3개로 가를 것.
	if (m_start_small > m_end_big) {
		IS_Swap(m_start_small, m_end_big);
	}							//구조체배열GETLottoWinningNumberLookup
	m_start_small -= 1;			//-1이유 : 구조체배열에 접근시 시작이 0부터 시작하므로

	int index = 0;	int total = 0;
	int count[LOTTO_1to45_LENTH] = { 0 };

	int robot_num = 0;
	if (m_start_small == 0) {
		robot_num = 1;
	}
	else if (m_start_small == 261) {
		robot_num = 2;
	}
	else if (m_start_small == 732) {
		robot_num = 3;
	}
	printf("\n\n\n============%d세대 (%d회~%d회) 로또 당첨번호 분포통계표============\n\n",
		robot_num, m_start_small + 1, m_end_big);
	for (int i_range = m_start_small; i_range < m_end_big; i_range++) {
		for (int i_6 = 0; i_6 < 6; i_6++) {
			index = GETLottoWinningNumberLookup(i_range).SixNumber[i_6] - 1;
			count[index]++;
			SETLotto45DistributionLookup_Ball(index, count[0]);
			total++;
		}
	}
	int Ball_total = 0;
	int Ball_per_total = 0;
	float per = 0.0;
	int per_int = 0;
	float per_total = 0.0;

	//IS_PRINT_LOTTO_ARRAY(int array_1th_Lotto[])
	for (int i_45 = 0; i_45 < LOTTO_1to45_LENTH; i_45++) {
		Ball_total += SETLotto45DistributionLookup_Ball(i_45, -1).ball;
		per = ((float)SETLotto45DistributionLookup_Ball(i_45, -1).ball / total) * 100;
		per_total += per;
		Ball_per_total += per * 100;
		printf("(%2d번) = [%2d회, ", i_45 + 1, SETLotto45DistributionLookup_Ball(i_45, -1).ball);
		printf("%.2f%%] ", ((float)SETLotto45DistributionLookup_Ball(i_45, -1).ball / total) * 100);
		IS_PRINT_NEWLINE(i_45, 3);
	}
	printf("\n");
}
...
void Menu_SELECT(int m_cost)
{
	switch (m_cost)
	{
	case 1:			//1 ------- 1세대부터 3세대까지 확률 보기
		IS_Print_Nth_Lotto(LOTTO_MACHIN_1TH);
		IS_Print_Nth_Lotto(LOTTO_MACHIN_2TH);
		IS_Print_Nth_Lotto(LOTTO_MACHIN_3TH);
		//VIEW_Back_Menu();
		break;
	case 2:			//2 ------- 1세대랑 3세대 비교
		number_compare1();
		//VIEW_Back_Menu();
		break;
	case 3:			//3 ------- 2세대랑 3세대 비교
		number_compare2();
		//VIEW_Back_Menu();
		break;
	case 4:			//4 ------- 로또 티켓 구매하기
...
//void IS_Print_Array(int m_Array[],int size)
//{
//	for (int i = 0; i < LOTTO_SIX_NUMBER; i++)
//	{
//		printf(GREEN"%2d,", m_Array[i]);
//	}
//	printf("\n");
//}
...
void SET_Lotto45_Distribution_Lookup(int m_start_small, int m_end_big)
{
	...
	//GET_Lotto_Stack_Count(int array_1th_Lotto[])
	printf("==========%d회차 부터%5d회차까지 로또 당첨번호 분포통계표==========\n",
		m_start_small + 1, m_end_big);
	for (int i_range = m_start_small; i_range < m_end_big; i_range++) {
		for (int i_6 = 0; i_6 < 6; i_6++) {
			index = GETLottoWinningNumberLookup(i_range).SixNumber[i_6] - 1;
			count[index]++;
...
void Get_Numbers_Do_Not_Overlap(int member_Array[], int m_range) {	//랜덤 값을 생성해서 중복 값을 피하는 함수
	int index = 0;	int flag = 0;
	int m_found_Flag = 0;	int for_Value = 0;
	///printf("IN Get_Numbers_Do_Not_Overlap\n");

	for (int i = 0; i < LOTTO_1to45_LENTH; i++) {
		index = Get_Random_Number(m_range); //랜덤 값을 받아서 temp변수에 저장
		member_Array[i] = index; //랜덤 값을 받아서 temp변수에 저장
		m_found_Flag = 0;
		for_Value = 0;
		while (for_Value <= LOTTO_1to45_LENTH) {
			if (member_Array[for_Value] != index) {	//member_Array[finding]배열에 저장되어 있는 수
														//member_temp 생성한 난수
				flag = TRUE;							//**found_Flag에 1을 저장
				for_Value++;
			}
			else {										// 난수와 배열값이 같지 않다면
				index = Get_Random_Number(m_range);		//새로운 랜덤값을 받아서 다시 temp에 저장
				for_Value = -1;							//새로운 랜덤값이 없는지 재확인.
			}
		}//for 끝
		if (flag == TRUE) {
			member_Array[i] = index;
		}
	}//for 끝
}

void number_compare1() {
	float first_third_compare = 0.0;
	int first_g[45] = { 36,40,40,41,35,34,36,31,22,32,
		36,29,33,37,31,32,38,38,40,36,35,26,32,33,39,
		37,37,29,30,34,37,29,36,37,37,41,45,26,37,43,28,38,34,35,34 }; // 1세대 공 갯수 데이터

	int third_g[45] = { 20,24,25,18,24,25,21,20,22,26,24,37,
		26,26,23,31,27,33,30,12,34,21,19,27,18,25,24,29,20,24
		,26,21,28,31,21,24,19,32,32,18,24,31,34,21,31 }; // 3세대 공 갯수 데이터

	for (int i = 0; i < 45; i++) {
		float first_g_per = (float)first_g[i] / FIRST_th_PER * 100;

		float third_g_per = (float)third_g[i] / THIRD_th_PER * 100;
	}

	//제목
	printf("\n");
	printf("=======1세대랑 3세대 로또 당첨번호 확률비교표=======\n\n");

	for (int i = 0; i < 45; i++) {

		first_third_compare = (float)first_g[i] / FIRST_th_PER * 100 - (float)third_g[i] / THIRD_th_PER * 100;
		if (first_third_compare < 0)
		{
			first_third_compare = first_third_compare * -1;
		}
		printf(" (%2d번) = [%.3f%%] ", i + 1, first_third_compare);
...
void number_compare2() {
	...
	float second_third_compare = 0.0;
	int second_g[45] = { 73,57,56,65,68,57,65,73,50,67,65,
		65,69,68,68,57,68,62,58,80,55,56,58,62,61,63,75,56,
		61,53,61,57,65,76,55,55,63,65,62,71,58,50,67,65,65 }; // 2세대 공 갯수 데이터
	...
	for (int i = 0; i < 45; i++) {
		float second_g_per = (float)second_g[i] / SECOND_th_PER * 100;
		float third_g_per = (float)third_g[i] / THIRD_th_PER * 100;
	}
	//제목
	printf("\n");
	printf("=======2세대랑 3세대 로또 당첨번호 확률비교표=======\n\n");
	for (int i = 0; i < 45; i++) {
		second_third_compare = (float)second_g[i] / SECOND_th_PER * 100 - (float)third_g[i] / THIRD_th_PER * 100;
		if (second_third_compare < 0)
		{
			second_third_compare = second_third_compare * -1;
		}
		printf(" (%2d번) = [%.3f%%] ", i + 1, second_third_compare);
		if (i % 3 == 2) {
			printf("\n");
		}
	}
...
void WORKING(int m_start_small, int m_end_big) {
	...
	//printf("==========%d회차 부터%5d회차까지 로또 당첨번호 분포통계표==========\n",
		//m_start_small + 1, m_end_big);
	for (int i_range = m_start_small; i_range < m_end_big; i_range++) {
		//printf("%3d회차: ", i_range + 1);
		for (int i_6 = 0; i_6 < 6; i_6++) {
			//index = GETLottoWinningNumberLookup(i_range).SixNumber[i_6] -1;
			index = GETLottoWinningNumberLookup(i_range).SixNumber[i_6];
			count[index - 1]++;
			//SETLotto45DistributionLookup_Ball(index-1, count[0]);
			SETLotto45DistributionLookup_Ball(index - 1, count[index - 1]);
			total++;
			//printf("%2d, ", index);
			//Ball_total += index;
		}
	...
	for (int i_45 = 0; i_45 < LOTTO_1to45_LENTH; i_45++)
	{
		...
		per = ((float)SETLotto45DistributionLookup_Ball(i_45, -1).ball / total) * 100;
		per_total += per;
		//printf("[%02d]공:%3d회, ", i_45 + 1, SETLotto45DistributionLookup_Ball(i_45, -1).ball);
		//printf("%.2f%% ", ((float)SETLotto45DistributionLookup_Ball(i_45, -1).ball / total) * 100);
		//IS_PRINT_NEWLINE(i_45, 3);
	}
	//내게 필요한 코드는 printf("[%02d]공:%3d회, ", i_45, SETLotto45DistributionLookup_Ball(i_45, -1).ball);
	// total 변수	1128
	//printf(">>>>>>>>>>>(Ball_total):%6d \n", Ball_total);
	//printf(">>>>>>>>>>>(per_total):%f \n", per_total);
	//printf(">>>>>>>>>>>(TOTAL):%6d \n", total);
...
//	//printf("할일 리스트\n");
//	for (int i = 0; i < 45; i++) {
//		//per = SETLotto45DistributionLookup_Ball(i, -1).ball * BEA_SU;
//		//printf("%d번공 을 %d개 만큼 적기\n", i + 1, SETLotto45DistributionLookup_Ball(i, -1).ball);
//	}
	//printf("총 리스트의 공의수는 %d개 임.\n", total);
	//상자를 준비하고ok
	//		//준비해둔 리스트를 꺼내고 ok
	//		//공에 숫자를 적고
	//		//공을 상자에 넣고
	//		//상자에 넣은 공의 숫자를 바꾸고. 올리고.

	//printf("=======SETLotto45DistributionLookup_Ball=========\n");
	int array_box[1128] = { 0 };
	int array_box_index = 0;
	//		//공에 숫자를 적고
	//		//공을 상자에 넣고
	for (int i_45 = 0; i_45 < 45; i_45++)
	{
		int count = SETLotto45DistributionLookup_Ball(i_45, -1).ball;
		for (int j = 0; j < count; j++)
		{
			array_box[array_box_index] = i_45 + 1;
			array_box_index++;
		}
	}
	//for (int i_1128 = 0; i_1128 < 1128; i_1128++)
	//{
	//	//printf("%d ", array_box[i_1128]);
	//	//IS_PRINT_NEWLINE(i_1128, 5);
	//}
	int array_LOTTO[6] = { 0 };
	//printf("Get_Numbers_Do_Not_Overlap\n");

	//Get_Numbers_Do_Not_Overlap(array_LOTTO, 6);
	getDuplicateLottoNumber(array_LOTTO, array_box, 1128);


	//printf("IS_Select_Sorting_Ascending\n");
	IS_Select_Sorting_Ascending(array_LOTTO);

	//printf("IS_PRINT_Array\n");
	IS_PRINT_Array(array_LOTTO, 6);

	//printf("\n ball_count : %2d, ", ball_count);
	//printf("\n ball_count : %2d, ", ball_count);

	//IS_PRINT_LOTTO_ARRAY(int array_1th_Lotto[])

	//printf("남은 공은 21개이니. 9980번부터 수를 입력합니다. 난수를 받아서... \n", Ball_per_total);
	//printf("만들어진 상자에서 랜덤으로 하나 꺼내는 방법.. \n", Ball_per_total);
	// 랜덤 인덱스(배열길이 까지의 나머지를 구하고.)
	// 이를 20번 접근해서
	// 뒷자리 부터 채워
	// 총 10000개의 공을 만든다.
	// 준비물, 1. 미완성된 상자만들어야 함., 2.상자의 현재 공의 숫자

}

void getDuplicateLottoNumber(int m_lottoArray[], int m_BoxArray[], int m_range)
{

	int temp = 0;
	int found = 0;
	temp = m_BoxArray[Get_Random_Number(m_range) - 1];
	m_lottoArray[0] = temp;
	...
	for (int i = 0; i < LOTTO_SIX_NUMBER; i++)
	{
		temp = m_BoxArray[Get_Random_Number(m_range) - 1];
		//printf("i: %d  temp: %d\n", i, temp);
		for (int finding = 0; finding < i; finding++)
		{
			if (m_lottoArray[finding] == temp)
			{
				//printf("전 m_lottoArray[%d] : %d == temp: %d\n",	
					//finding, m_lottoArray[finding], temp);
				temp = m_BoxArray[Get_Random_Number(m_range) - 1];
				finding = 0;
				//printf("후 m_lottoArray[%d] : %d == temp: %d\n",	
					//finding, m_lottoArray[finding], temp);
			}
			else
			{
				//printf("getDuplicateLottoNumber else\n");
				//printf("m_lottoArray[%d] : %d != temp: %d\n",
					//finding, m_lottoArray[finding], temp);
				//IS_PRINT_Array(m_lottoArray,  5);
				found = 1;
			}
			//getchar();
		}
...
int Get_Random_Number(int m_range)	//랜덤 값을 생성해서 int형으로 반환
{
	//	#include <stdlib.h>
	//	#include <time.h>
	//srand((unsigned)time(NULL)); // 난수를 초기화하는 srand 호출
	return rand() % m_range + 1;
}

void IS_Select_Sorting_Ascending(int m_Array[])
{
	//printf("isSortingAscending\n");
	int temp = 0;
	int found = 0;
	int temp_Array = { 0 };
	//선택정렬
	...
	for (int i = 0; i < LOTTO_SIX_NUMBER; i++)
	{
		for (int finding = 0; finding < LOTTO_SIX_NUMBER; finding++)
		{
			if (m_Array[i] < m_Array[finding])
			{
				//swap
				temp = m_Array[i];
				m_Array[i] = m_Array[finding];
				m_Array[finding] = temp;
			}
		}
	}
	//putchar('\n');
...
void main()
{
	...
	//WORKING(LOTTO_MACHIN_3TH);
	//number_compare2(LOTTO_MACHIN_2TH);
}

T_LottoWinningNumber GETLottoWinningNumberLookup(int lotto_round) {
	//인자설명 : lotto_round 는 가져올 회차.
	//T_data* heap_LottoNum;
	//heap_LottoNum = (T_data *)malloc(sizeof(T_data)* LOTTO_SIX_NUMBER);
	//free(heap_LottoNum);

	T_LottoWinningNumber LottoWinningNumber[LOTTO_ROUND] = {
	{1,10,23,29,33,37,40},
	{2,9,13,21,25,32,42},
	{3,11,16,19,21,27,31},
	{4,14,27,30,31,40,42},
	...
	return LottoWinningNumber[lotto_round];
}

static T_Box g_box[LOTTO_1to45_LENTH];

T_Box GETLotto45DistributionLookup(int lotto_round) {
	return g_box[lotto_round];
	//T_data* heap_LottoNum;
	//heap_LottoNum = (T_data *)malloc(sizeof(T_data)* LOTTO_SIX_NUMBER);
	//free(heap_LottoNum);
}
T_Box SETLotto45DistributionLookup_Ball(int ball_num, int count) {
	...
	if (count > 0) {
		g_box[ball_num].ball = count;
		//printf("구조체box[%d].ball = %d \n",ball_num, count);
	}
	else if (count < 0) {
		return g_box[ball_num];
	}
	else {
		return;
	}
